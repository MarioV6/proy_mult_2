...
const saveNote = async () => {
  if (selectedDay !== null) {
    ...
    const dataToSend = {
      fecha: key,
      note: currentText,
      usuario_id:  // O el ID del usuario actual
    };

    try {
      const response = await fetch('http://localhost:8000/apps/users/application/save_note/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        // Si el servidor responde bien, actualizamos el estado local
        setNotes({ ...notes, [key]: currentText });
        setSelectedDay(null);
        setCurrentText("");
        console.log("Guardado en la DB con éxito");
      }
    } catch (error) {
